...
vdp_ymmm_size   equ     0000Eh  ; Number of bytes required to perform a YMMM
...
; Update posters using diffblit and vdp commands 
...
turtles_slide3_middle_blit:
        ...
        jp      return_irq_exx
1:
        ld      hl, poster_slide4_diff
        ld      (slide_data), hl
        ld      hl, poster_slide4_size
        ld      (slide_size), hl
        ld      hl, poster_slide4_cmd
        ld      (slide_command), hl
        jp      turtles_slide4_middle_blit
 
; ----------------------------------------------------------------
; State: turtles_slide4
; Update posters using diffblit and two HMMM vdp commands 
; during vertical retrace.
...
turtles_slide4:
        ...
        VDP_STATUS 1
        ENABLE_HIRQ
        NEXT_HANDLE turtles_slide4_middle
        call    update_poster_command
        QUEUE_VDP_COMMAND bottom_poster_cmd
        ld      hl, (slide_data)
        call    queue_diffblit
...
turtles_slide4_middle:
        ; Order is important!
        PREAMBLE_HORIZONTAL
        DISABLE_SCREEN
        exx
        ld      hl, bottom_palette 
        call    queue_short_palette
        ADD_VAR vscroll_bottom, 4
        HSPLIT_LINE 106
        NEXT_HANDLE turtles_slide_bottom
...
turtles_slide4_middle_blit:
        ...
        jp      return_irq_exx
1:
        ld      hl, poster_slide5_diff
        ld      (slide_data), hl
        ld      hl, poster_slide5_size
        ld      (slide_size), hl
        ld      hl, poster_slide5_cmd
        ld      (slide_command), hl
        jp      turtles_slide5_middle_blit

; ----------------------------------------------------------------
; State: turtles_slide5
; Update posters using diffblit and two YMMM vdp commands 
; during vertical retrace.
...
turtles_slide5:
        ...
        VDP_STATUS 1
        ENABLE_HIRQ
        NEXT_HANDLE turtles_slide5_middle
        call    update_poster_command
        QUEUE_VDP_COMMAND bottom_poster_cmd
        ld      hl, (slide_data)
        call    queue_diffblit
...
turtles_slide5_middle:
        ...
        COMPARE_FRAME 1159
        jp      z, return_irq_exx
turtles_slide5_middle_blit:
        QUEUE_VDP_COMMAND top_poster_cmd
        ld      hl, (slide_data)
        call    queue_diffblit
        call    update_slide_data
        QUEUE_VDP_COMMAND_IND slide_command
        ADD_VARW slide_command, vdp_ymmm_size
        ld      hl, (slide_data)
        call    queue_diffblit
        call    update_slide_data
        QUEUE_VDP_COMMAND_IND slide_command
...
slide_command:          dw      poster_slide3_cmd 
state_end:
...
poster_slide4_diff:     incbin "poster_slide4_diff.d5"
poster_slide5_diff:     incbin "poster_slide5_diff.d5"
poster_slide4_size:     incbin "poster_slide4_size.bin"
